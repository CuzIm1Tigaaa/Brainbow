...
public class Events implements Listener {
	...
	public void shootBow(EntityShootBowEvent event) {
		...
		switch(block.getType()) {
			case REDSTONE_BLOCK -> {
				...
				tnt.setFuseTicks(tnt.getFuseTicks() * 2 / 3);
			...
			case EMERALD_BLOCK -> {
				...
				potionMeta.setColor(PotionEffectType.POISON.getColor());
			...
			case IRON_BLOCK -> {
				...
				int numArrows = 10, angle = 10;
				Vector direction = arrow.getVelocity();

				double offsetAngle = Math.toRadians(angle);
				double verticalOffset = Math.toRadians(angle);

				for(double i = -offsetAngle; i <= offsetAngle; i += offsetAngle / 2) {
					for(double j = -verticalOffset; j <= verticalOffset; j += verticalOffset / 2) {
						if((int) (offsetAngle * 1000) == 0 && (int) (verticalOffset * 1000) == 0)
							continue;
						shootAdditionalArrow(player, direction, i, j);
					}
				}
			}
			case COAL_BLOCK -> {
			...
			case NETHERITE_BLOCK -> {
				Arrow arrow = player.getWorld().spawn(player.getEyeLocation(), Arrow.class);
				arrow.setVelocity(event.getProjectile().getVelocity());
				arrow.addCustomEffect(new PotionEffect(PotionEffectType.LEVITATION, 30, 9), true);
	...
	private void shootAdditionalArrow(Player player, Vector direction, double horizontalOffset, double verticalOffset) {
		Vector newDirection = direction.clone();

		if(horizontalOffset != 0) {
			newDirection.rotateAroundY(horizontalOffset);
		}

		if(verticalOffset != 0) {
			newDirection.rotateAroundZ(verticalOffset);
			newDirection.rotateAroundX(verticalOffset);
		}

		Arrow spread = player.launchProjectile(Arrow.class);
		spread.setMetadata("multishot", new FixedMetadataValue(plugin, true));
		spread.setVelocity(newDirection);
	}
	...
	public void move(PlayerMoveEvent event) {
		...
		player.addPotionEffect(new PotionEffect(PotionEffectType.SLOW, PotionEffect.INFINITE_DURATION, 4, false, false, false));
	}
